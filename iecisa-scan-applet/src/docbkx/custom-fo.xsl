...
<xsl:stylesheet xmlns:t="http://nwalsh.com/docbook/xsl/template/1.0"
    ...
    <xsl:param name="admon.graphics.path" select="concat($basedir,'/src/docbkx/images/')"></xsl:param>
    ...
    <xsl:param name="front.cover">
        <xsl:value-of select="concat($basedir,'/src/docbkx/images/front-cover.jpg')"/>
    </xsl:param>
        ...
        <fo:page-sequence master-reference="back-cover">
            <fo:flow flow-name="xsl-region-body">
                <fo:block margin-top="240mm" margin-left="85mm" background-color="white">
                    <xsl:variable name="back.cover.image" select="concat($basedir,'/src/docbkx/images/logo-ieci.png')" />
